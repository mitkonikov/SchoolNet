...
export const DB = function(database: string) {
    ...
    let network_table = function(table) {
        ...
        let playGame = (gameID, classID, teacherID, callback) => {
            ...
            let CURRENT_DATE_TIME = new Date().toISOString().slice(0, 19).replace('T', ' ') as any;
    ...
    let DBSelector = {
        "db_net": networkAPI,
        "db_words": databases.db_words,
        "db_records": databases.db_records,
        "db_znam": databases.db_znam
    }
