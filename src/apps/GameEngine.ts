...
import { setUpDemo } from "./DemoLogger";
...
let buildGameEngine = (databaseController_connect) => {
    if (!built) {
        dbController = databaseController_connect;
        network = databaseController_connect.DB("db_net");
...
let setUpGame = (socket, callback) => {
    ...
    dbController.DB("db_net").table("tbl_games_current").getCurrentGame.TeacherID(1, USER.ID, (rows) => {
        ...
        // Set up the demo
        setUpDemo(network, rows[0].Demo_ID, rows[0].Game_ID, rows[0].Room_ID);
    });
}
