...
export const Guest = (API: TAPI) => {
    ...
    if (typeof cookieName == "undefined" || cookieName == '') {
        console.trace("Guest session cookie name not set!");
        return;
    }

    // Guest middleware for Express.JS
    API.app.use((req: IRequest, res, next) => {
        ...
        if (typeof req.cookies[cookieName] != "undefined") {
            ...
            // TODO: make a wrapper for the database storage
            API.network.query("INSERT INTO tbl_guests SET ?", guest, (err, rows) => {
                guest.ID = rows.insertId;
                guest.Display_Name = '';
    ...
    API.app.get('/play/guest', (req: IRequest, res) => {
        res.send(req.guest);
    });
}